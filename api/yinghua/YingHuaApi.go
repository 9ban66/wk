...
const yingHuaHTTPTimeout = 12 * time.Second

func newIPv4YingHuaTransport() *http.Transport {
	dialer := &net.Dialer{
	...
	return &http.Transport{
		...
		ResponseHeaderTimeout: yingHuaHTTPTimeout,
		ExpectContinueTimeout: time.Second,
		IdleConnTimeout:       yingHuaHTTPTimeout,
...
func CourseVideListApi(cache YingHuaUserCache, courseId string /*课程ID*/, retry int, lastError error) (string, error) {
	...
	if strings.Contains(string(body), "502 Bad Gateway") || strings.Contains(string(body), "504 Gateway Time-out") || strings.Contains(string(body), `"status":false,"_code":500`) {
		res.Body.Close()                   //立即释放
		time.Sleep(time.Millisecond * 150) //延迟
		return CourseVideListApi(cache, courseId, retry-1, fmt.Errorf("%s", string(body)))
	}
